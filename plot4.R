...
op <- par(mfrow=c(2,2))
with(myData, plot(TimeStamp, Global_active_power, type="l", ylab="Global Active Power", xlab=""))
with(myData, plot(TimeStamp, Voltage, type="l", ylab="Voltage", xlab="datetime"))
...
with(myData, plot(TimeStamp, Global_reactive_power, type="l", ylab="Global_reactive_power", xlab="datetime"))
op <- par(op)
dev.off()
